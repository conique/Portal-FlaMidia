        ...
        <div class="row mb-3">
            ...
            <div class="col-md-6">
                ...
                <select class="form-select @error('status') is-invalid @enderror" id="status" name="status">
                    ...
                    <option value="Em Andamento" {{ old('status', $championship->status) == 'Em Andamento' ? 'selected' : '' }}>Em Andamento</option>
                    <option value="Finalizado" {{ old('status', $championship->status) == 'Finalizado' ? 'selected' : '' }}>Finalizado</option>
                    <option value="Próximos Jogos" {{ old('status', $championship->status) == 'Próximos Jogos' ? 'selected' : '' }}>Próximos Jogos</option>
                </select>
                @error('status')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
